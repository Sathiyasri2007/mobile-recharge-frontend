...
const buttonClickStyle = `
  ...
  .start-recharge-btn:hover {
    transform: translateY(-3px) scale(1.05) !important;
    box-shadow: 0 10px 25px rgba(227, 83, 54, 0.4) !important;
    animation: none !important;
  }
  .start-recharge-btn:active {
    transform: translateY(-1px) scale(1.02) !important;
  }
  @keyframes pulse {
    0% { box-shadow: 0 4px 6px #A0522D; }
    50% { box-shadow: 0 6px 12px rgba(227, 83, 54, 0.6); }
    100% { box-shadow: 0 4px 6px #A0522D; }
  }
  .home-card {
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-light" style={{backgroundImage: 'url(/images/bg-pattern.svg)', backgroundRepeat: 'repeat', backgroundSize: '100px 100px'}}>
      <style>{buttonClickStyle}</style>
      {/* Hero Section */}
      <section className="bg-light py-8 relative" style={{backgroundImage: 'linear-gradient(135deg, #F5F5DC 0%, #F4A460 50%, #E35336 100%)', backgroundSize: 'cover'}}>
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-6xl font-bold text-secondary mb-6">Power Up Your Mobile.</h1>
              <p className="text-2xl text-primary mb-8">Fast, secure prepaid recharge in seconds.</p>
              <Link 
                to="/plans" 
                className="bg-primary text-light px-8 py-4 rounded-lg font-semibold hover:bg-secondary transition-colors shadow-lg inline-block start-recharge-btn"
              >
                Start Recharging
              </Link>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {services.map((service) => (
              <button
                ...
                className="bg-accent rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-shadow focus:outline-none focus:ring-2 focus:ring-secondary cursor-pointer click-animate home-card"
              >
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-2xl mx-auto">
            {operators.map((operator, index) => (
              <div key={index} className="bg-light rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-shadow home-card">
                <img 
                  src={operator.logo} 
                  alt={operator.name}
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="bg-light rounded-lg p-6 max-w-md mx-auto">
            ...
            <div className="space-y-2">
              <Link 
                to="/plans" 
                className="block bg-secondary text-light px-6 py-2 rounded font-semibold hover:bg-primary transition-colors start-recharge-btn"
              >
                Start Earning
              </Link>
              <button 
                onClick={() => setShowOffers(true)}
                className="block w-full bg-primary text-light px-6 py-2 rounded font-semibold hover:bg-accent hover:text-secondary transition-colors start-recharge-btn"
              >
                Apply Offer
              </button>
            </div>
      ...
      <section className="py-16 bg-secondary text-light">
        <div className="container mx-auto px-4 text-center">
          ...
          <Link 
            to="/plans" 
            className="bg-primary text-light px-8 py-3 rounded-lg font-semibold hover:bg-accent hover:text-secondary transition-colors shadow-lg start-recharge-btn"
          >
            Get Started Now
          </Link>
        </div>
